...
public void searchAndExportForwardIndirectIntermediaryReport() {
	...
	public boolean verifyCurriculumStandardDataUI(Map<String, List<String>> RepMap, ExtentTest logger) {
		boolean flag = false;
		JavascriptExecutor jse = (JavascriptExecutor) driver;
		WebDriverWait wait = new WebDriverWait(driver, 120);
		...
		try {
			...
			csList = RepMap.get("CurriculumStandard");
	...
	public boolean verifyIntermediaryDataUI(Map<String, List<String>> productTIRepMap, ExtentTest logger) {
		...
		try {
			...
			commonPOM.getPearsonLogo().click();
			commonPOM.getSchoolGlobalLOB().click();
			this.searchIntermediaryDiscipline();
					...
					if (!webElement.isEmpty()) {
						...
						while (itr.hasNext()) {
							...
							if (structureName.contains(dis)) {
								flag = true;
								continue;
							} else if (dis.equalsIgnoreCase(ReportsConstant.INGESTED_INTERMEDIARY)){
								flag = false;
							}
							 else {
									flag = false;
	...
	public String createAndDownloadReport(String reportType) {
	String reportName = null;
	...
	public void clickIngestedIntermediaryDiscipline(String pivot) {
		...
		public void searchIntermediaryDiscipline() {
